...
const goBTN = document.querySelector('.ir');
...
const showMovies = async () => {
  ...
  pageNumber.max = Number(movies.total_pages);
  setPaginationState();
  renderCards(movies.results);
};

const setPaginationState = () => {
  pageNumber.value = appState.page;
...
const isPageBetween = (pageNum) => {
  return appState.total >= pageNum && pageNum > 0;
};

const goToPage = () => {
  const pageNum = Number(pageNumber.value);
  if (pageNum < 1 || pageNum === appState.page) return;
  if (pageNum > appState.total) appState.page = appState.total;
  if (isPageBetween(pageNum)) appState.page = pageNum;
  changePage();
};
